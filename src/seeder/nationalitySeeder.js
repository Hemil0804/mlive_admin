...
module.exports = {
    run: () =>
        new Promise((resolve) => {
            (async() => {
                let nationalities = nationalityArray;
                let filterNationality = nationalities.map(function({ nationality }) {
                    return { nationality };
                });
                for (let a of filterNationality) {
                    let nationalityFound = await nationalityModel.findOne({ nationality: a.nationality, status: 1 })
                    if (!nationalityFound) await nationalityModel.updateOne(a, { $set: a }, { upsert: true });
                }
                resolve(true);
            })();
        })
